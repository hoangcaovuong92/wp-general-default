...
?>
	...
	while(have_posts()) :
		the_post(); TvlgiaoWpdanceClassNameVar::TVLGiao_wpdance_GetVar("post"); $wp_query = TvlgiaoWpdanceClassNameVar::TVLGiao_wpdance_GetVar("wp_query");
			...
			<div class="item-content<?php echo is_sticky()? ' sticky' : ''?>">
				...
				<div class="meta-post post-info-content">
					...
					<div class="post-info-meta-top post-info-meta">
						...
						else echo absint($comments_count->approved) . ' ' . _n( "Comment", "Comments", absint($comments_count->approved), 'wpgeneral');?>
						</div>
						<?php endif;?>
					</div>
					...
					<p class="excerpt"><?php 
						...
						tvl_wpdance_the_excerpt_max_words($words_limit,$post); ?>
					</p>
					<?php endif;?>
					<?php if( $tvlgiao_wpdance_wd_data['wd_blog_readmore'] == 1 ) : ?>		
					<a class="button" href="<?php the_permalink(); ?>"><?php esc_html_e("Read more", "wpgeneral");?></a>
					<?php endif;?>
