...
//add filter hook
add_filter('woocommerce_widget_cart_product_title','tvlgiao_wpdance_add_sku_after_title',10,2);
...
add_action( 'woocommerce_before_shop_loop_item_title', 'tvlgiao_wpdance_wd_template_loop_product_thumbnail', 10 );

add_action( 'wd_woocommerce_shop_loop_buttons', 'tvlgiao_wpdance_wd_list_button_div_box_start', 13 );
add_action( 'wd_woocommerce_shop_loop_buttons', 'tvlgiao_wpdance_wd_list_template_loop_add_to_cart', 14 );
add_action( 'wd_woocommerce_shop_loop_buttons', 'tvlgiao_wpdance_wd_list_button_div_box_end', 25 );


function tvlgiao_wpdance_wd_list_button_div_box_start(){
	global $tvlgiao_wpdance_wd_data;
	$style = (isset($tvlgiao_wpdance_wd_data['wd_prod_button_style']) && absint($tvlgiao_wpdance_wd_data['wd_prod_button_style']))? 'style1': 'style2';
...
add_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_sharing', 33 );
...
function tvlgiao_wpdance_remove_excerpt_from_list(){
	if(class_exists('woocommerce') && (is_tax( 'product_cat' ) || is_shop() )){
		...
		remove_action ('woocommerce_after_shop_loop_item','tvlgiao_wpdance_add_short_content',5);
		add_action( 'woocommerce_after_shop_loop_item', 'tvlgiao_wpdance_wd_list_template_loop_add_to_cart', 8 );
		//add_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_single_excerpt', 9);
	} else {
		add_action( 'woocommerce_after_shop_loop_item_title', 'tvlgiao_wpdance_wd_list_template_loop_add_to_cart', 10 );
...
function tvlgiao_wpdance_wd_list_template_loop_add_to_cart(){
	 global $tvlgiao_wpdance_wd_data;
	if (isset($tvlgiao_wpdance_wd_data['wd_prod_button_style']) && absint($tvlgiao_wpdance_wd_data['wd_prod_button_style']) && !wp_is_mobile()) {
...
function tvlgiao_wpdance_add_short_content($num_words=0){
	...
	$rs = '';
	$rs .= '<div class="product_short_content">'.$num_words ;
...
function tvlgiao_wpdance_wd_template_loop_product_thumbnail(){
	global $product;
...
if ( ! function_exists( 'tvlgiao_wpdance_wd_subcategory_thumbnail' ) ) {
	...
	function tvlgiao_wpdance_wd_subcategory_thumbnail( $category ) {
		$small_thumbnail_size  	= apply_filters( 'single_product_small_thumbnail_size', 'tvlgiao_wpdance_sub_categories_thumbnail' );
...
function tvlgiao_wpdance_add_label_to_product_list(){
	...
	if ($product->is_on_sale()){ 
		if( $product->get_regular_price() > 0 ){
			...
			echo "<span class=\"onsale product_label\">".esc_html__( 'Save','wpgeneral' )."</span>";
		}
	}
	elseif ($product->is_featured()){
		echo "<span class=\"featured product_label\">".esc_html__( 'New','wpgeneral' )."</span>";
	}
	echo "</div>";
}
...
if( class_exists( 'YITH_Woocompare_Frontend' ) && class_exists( 'YITH_Woocompare' ) ) {
	...
	function tvlgiao_wpdance_wd_add_compare_link( ) {
        global $yith_woocompare,$product; $fontend = $yith_woocompare->obj;
        ...
        if ( ! isset( $button_text ) || $button_text == 'default' ) {
            $button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Compare', 'wpgeneral' ) );
	...
	function tvlgiao_wpdance_wd_add_compare_button1(){
	 ...
	 $yith_woocompare = TvlgiaoWpdanceClassNameVar::TVLGiao_wpdance_GetVar("yith_woocompare");
...
function tvlgiao_wpdance_wd_add_style_yith_compare(){
	...
	wp_register_style( 'tvlgiao-wpdance-yith_compare', TVLGiao_Wpdance_THEME_CSS.'/yith_compare.css');
	wp_enqueue_style('tvlgiao-wpdance-yith_compare');	
	wp_register_script( 'tvlgiao-wpdance-yith-compare-js', TVLGiao_Wpdance_THEME_JS.'/yith_compare.js',false,false,true);
	wp_enqueue_script('tvlgiao-wpdance-yith-compare-js');
...
function tvlgiao_wpdance_wd_addon_product_tabs( $tabs = array() ){
		...
		if ( $post->post_excerpt )
			$tabs['description'] = array(
				'title'    => esc_html__( 'Description', 'wpgeneral' ),
				'priority' => 10,
		...
		if ( comments_open() && $tvlgiao_wpdance_wd_data['wd_prod_review'] )
			$tabs['reviews'] = array(
				'title'    => sprintf( esc_html__( 'Reviews (%d)', 'wpgeneral' ), get_comments_number( $post->ID ) ),
				'priority' => 90,
		...
		if ( $product->has_attributes() || ( get_option( 'woocommerce_enable_dimension_product_attributes' ) == 'yes' && ( $product->has_dimensions() || $product->has_weight() ) ) )
			$tabs['additional_information'] = array(
				'title'    => esc_html__( 'Additional Information', 'wpgeneral' ),
				'priority' => 20,
...
function tvlgiao_wpdance_wd_addon_custom_tabs ( $tabs = array() ){
	...
	if($tvlgiao_wpdance_wd_data['wd_prod_customtab']) {
		$tabs['wd_custom'] = array(
			'title'    =>  sprintf( esc_html__( '%s','wpgeneral' ), stripslashes(esc_html($tvlgiao_wpdance_wd_data['wd_prod_customtab_title'])) )
			,'priority' => 70
...
?>
	<div class="return-shipping">        
        <div class="content-quick">
			...
			<?php 
				echo esc_attr($title = sprintf( esc_html__( '%s','wpgeneral' ), stripslashes(esc_attr($tvlgiao_wpdance_wd_data['wd_prod_ship_return_title'])) ));
			?>
		</h3>
...
function tvlgiao_wpdance_wd_template_single_sku(){
	global $product;
	echo "<p class='wd_product_sku'>".esc_html__("sku: ","wpgeneral")."<span class=\"product_sku\">" . esc_attr($product->get_sku()) . "</span></p>";
}	
...
function tvlgiao_wpdance_shop_loop_prod_remove_action($data){
	if( isset($data['show_price']) && !(int)$data['show_image']){
		remove_action( 'woocommerce_before_shop_loop_item_title', 'tvlgiao_wpdance_wd_template_loop_product_thumbnail', 10 );
	}
	...
	if( isset($data['show_countdown']) && ((int)$data['show_countdown'] && in_array( 'wd_countdown/wd_countdown.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ))){
		global $tvlgiao_wpdance_WD_Countdown;
		add_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_open_div_style') , 1 );
		add_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_set_default_view') );
		add_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_close_div_style'), 1 );
		
		add_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_open_div_style') , 1 );
		add_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_set_default_view') );
		add_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_close_div_style'), 1 );
	} 
...
function tvlgiao_wpdance_shop_loop_prod_add_action(){
	...
	if(in_array( 'wd_countdown/wd_countdown.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )) {
		global $tvlgiao_wpdance_WD_Countdown;
		remove_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_open_div_style') , 1 );
		remove_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_set_default_view') );
		remove_action( 'woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_close_div_style'), 1 );
		
		remove_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_open_div_style') , 1 );
		remove_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_set_default_view') );
		remove_action( 'wd_small_woocommerce_after_shop_loop_item', array( $tvlgiao_wpdance_WD_Countdown, 'countdown_close_div_style'), 1 );
	}
...
if ( ! function_exists( 'tvlgiao_wpdance_dimox_shop_breadcrumbs' ) ) {
	...
	function tvlgiao_wpdance_dimox_shop_breadcrumbs( $args = array() ) {
		...
		$defaults = apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Home', 'breadcrumb', 'wpgeneral' ),
		) );
	...
	function tvlgiao_wpdance_wd_checkout_fields_form($checkout){
		$checkout->checkout_fields['account']    = array(
			'account_username' => array(
				'type' => 'text',
				'label' => esc_html__('Account username', 'wpgeneral'),
				'placeholder' => _x('Username', 'placeholder', 'wpgeneral'),
				'class' => array('form-row-wide')
				),
			'account_password' => array(
				'type' => 'password',
				'label' => esc_html__('Account password', 'wpgeneral'),
				'placeholder' => _x('Password', 'placeholder', 'wpgeneral'),
				'class' => array('form-row-first')
				),
			'account_password-2' => array(
				'type' => 'password',
				'label' => esc_html__('Account password', 'wpgeneral'),
				'placeholder' => _x('Comfirm Password', 'placeholder', 'wpgeneral'),
