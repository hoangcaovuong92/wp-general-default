 ...
 * @since WP0001
 */
 if(!function_exists ('tvlgiao_wpdance_theme_page_menu_args')){
 ...
 * @return int
 */
  if(!function_exists ('tvlgiao_wpdance_theme_excerpt_length')){
 ...
 */
  if(!function_exists ('tvlgiao_wpdance_theme_continue_reading_link')){
function tvlgiao_wpdance_theme_continue_reading_link() {
	return ' <a href="'. esc_url( get_permalink()) . '">' . wp_kses_post(__( 'Continue reading <span class="meta-nav">&rarr;</span>', 'wpgeneral' )) . '</a>';
}
  }

  if(!function_exists ('tvlgiao_wpdance_theme_auto_excerpt_more')){
 ...
 */
   if(!function_exists ('tvlgiao_wpdance_theme_custom_excerpt_more')){
...
function tvlgiao_wpdance_theme_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'wpgeneral' ); ?></em>
			<br />
		<?php endif; ?>
			...
			<div class="comment-meta commentmetadata">
				...
				<?php
					...
					printf( esc_html__( '%1$s', 'wpgeneral' ), get_comment_date() ); ?>
				</a>
				<?php edit_comment_link( wp_kses_post(__( ' - <i class="fa fa-pencil-square-o"></i>', 'wpgeneral' ), ' ' ));
				?>
	...
	<li class="post pingback">
		<p><?php esc_html_e( 'Pingback:', 'wpgeneral' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'wpgeneral' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}
endif;
if(!function_exists ('tvlgiao_wpdance_theme_remove_recent_comments_style')){
...
if(!function_exists ('tvlgiao_wpdance_enqueue_less_styles')){	
...
if(!function_exists ('tvlgiao_wpdance_my_theme_register_required_plugins')){
function tvlgiao_wpdance_my_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'   				=> get_template_directory() . '/framework/includes/plugins/wd-plugin.zip', // The plugin source
			'required' 				=> true, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '1.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
		...
		array(
			'name'     				=> 'WD Color Demo', // The plugin name
			'slug'     				=> 'wd_color_demo', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory() . '/framework/includes/plugins/wd_color_demo.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
		...
		array(
			...
			'source'   				=> get_template_directory() . '/framework/includes/plugins/wd_countdown.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '1.0.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
		...
		array(
			'name'     				=> 'WooCommerce Currency Converter', // The plugin name
			'slug'     				=> 'woocommerce-currency-converter-widget', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory() . '/framework/includes/plugins/woocommerce-currency-converter-widget.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
		...
		array(
			'name'     				=> 'Ultimate WooCommerce Brands PRO', // The plugin name
			'slug'     				=> 'mgwoocommercebrands', // The plugin slug (typically the folder name)
			'source'   				=> get_template_directory() . '/framework/includes/plugins/mgwoocommercebrands.zip', // The plugin source
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
			'version' 				=> '2.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation' 		=> false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation' 	=> false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
		...
		array(
			'name'     				=> 'Tabby Responsive Tabs', // The plugin name
			'slug'     				=> 'tabby-responsive-tabs', // The plugin slug (typically the folder name)
			'required' 				=> false, // If false, the plugin is only 'recommended' instead of required
		),
	 ...
	 */
	$config = array(
		'domain'       		=> 'wpgeneral',         	// Text domain - likely want to be the same as your theme.
		...
		'strings'      		=> array(
			...
			'installing'                       			=> esc_html__( 'Installing Plugin: %s', 'wpgeneral' ), // %1$s = plugin name
			'oops'                             			=> esc_html__( 'Something went wrong with the plugin API.', 'wpgeneral' ),
			'notice_can_install_required'     			=> _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.' ,'wpgeneral'), // %1$s = plugin name(s)
			'notice_can_install_recommended'			=> _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.' ,'wpgeneral'), // %1$s = plugin name(s)
			'notice_cannot_install'  					=> _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.','wpgeneral' ), // %1$s = plugin name(s)
			'notice_can_activate_required'    			=> _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.' ,'wpgeneral'), // %1$s = plugin name(s)
			'notice_can_activate_recommended'			=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.','wpgeneral' ), // %1$s = plugin name(s)
			'notice_cannot_activate' 					=> _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.' ,'wpgeneral'), // %1$s = plugin name(s)
			'notice_ask_to_update' 						=> _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.' ,'wpgeneral'), // %1$s = plugin name(s)
			'notice_cannot_update' 						=> _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.','wpgeneral' ), // %1$s = plugin name(s)
			'install_link' 					  			=> _n_noop( 'Begin installing plugin', 'Begin installing plugins','wpgeneral' ),
			'activate_link' 				  			=> _n_noop( 'Activate installed plugin', 'Activate installed plugins','wpgeneral' ),
			...
			'complete' 									=> esc_html__( 'All plugins installed and activated successfully. %s', 'wpgeneral' ), // %1$s = dashboard link
