		...
		<div class="area-inner1">
			<div class="area-content">
				<ul class="page_config_list">
					...
					<li class="">
						<p>
							<label><?php esc_html_e('Post type','wpgeneral');?> </label>
					...
					<li class="global_sub slider_sub slider_shortcode" style="display:none">
						<p>
							<label><?php esc_html_e('Post Thumbnail Shortcode','wpgeneral');?></label>
							<textarea rows="5" name="post_thumb_shortcode" id="post_thumb_shortcode"><?php echo  $post_thumb_shortcode; ?></textarea>
							<span class="description"><?php esc_html_e('Input "breadcrumb" or shortcode string for custom post thumbnail.', 'wpgeneral'); ?></span>
						</p>
					</li>
					
					<li class="global_sub slider_sub slider_audio" style="display:none">
						<p>
							<label><?php esc_html_e('Mp3 URL','wpgeneral');?> </label>
							<input type="text" name="audio_mp3" id="audio_mp3" value="<?php echo strlen($_post_config['audio_mp3'])? $_post_config['audio_mp3']: ''; ?>" />
							<span class="description"><?php esc_html_e('Input mp3 URL. This audio shows on the single post instead of the post thumbnail', 'wpgeneral'); ?></span>
						</p>
					</li>
					<li class="global_sub slider_sub slider_audio" style="display:none">
						<p>
							<label><?php esc_html_e('WAV URL','wpgeneral');?> </label>
							<input type="text" name="audio_wav" id="audio_wav" value="<?php echo strlen($_post_config['audio_wav'])? $_post_config['audio_wav']: ''; ?>" />
							<span class="description"><?php esc_html_e('Input wav URL. This audio shows on the single post instead of the post thumbnail', 'wpgeneral'); ?></span>
						</p>
					</li>
					<li class="global_sub slider_sub slider_audio" style="display:none">
						<p>
							<label><?php esc_html_e('OGG URL','wpgeneral');?> </label>
							<input type="text" name="audio_ogg" id="audio_ogg" value="<?php echo strlen($_post_config['audio_ogg'])? $_post_config['audio_ogg']: ''; ?>" />
							<span class="description"><?php esc_html_e('Input ogg URL. This audio shows on the single post instead of the post thumbnail', 'wpgeneral'); ?></span>
						</p>
					</li>
					
					<li class="global_sub slider_sub slider_video" style="display:none">
						<p>
							<label><?php esc_html_e('Video URL','wpgeneral');?> </label>
							<input type="text" name="video_url" id="video_url" value="<?php echo strlen($_post_config['video_url'])? $_post_config['video_url']: ''; ?>" />
							<span class="description"><?php esc_html_e('Input Youtube, Vimeo or Dailymotion video URL. This video shows on the single post instead of the post thumbnail', 'wpgeneral'); ?></span>
						</p>
					</li>
					
					<li class="global_sub slider_sub slider_video" style="display:none">
						<p>
							<label><?php esc_html_e('Video width','wpgeneral');?> </label>
							<input type="number" min="320" max="1170" name="video_width" id="video_width" value="<?php echo esc_attr($_post_config['video_width']); ?>" />
						</p>
					</li>
					<li class="global_sub slider_sub slider_video" style="display:none">
						<p>
							<label><?php esc_html_e('Video height','wpgeneral');?></label>
