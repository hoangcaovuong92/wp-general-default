...
<ul class="cart_list thanhdoi product_list_widget <?php echo esc_attr($args['list_class']); ?>">
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<li class="media <?php echo esc_attr($_cart_li_class = ($_index == 0 ? "first" : ($_index == $_cart_array - 1 ? "last" : ""))); ?>">
						...
						<div class="cart_item_wrapper">
							...
							<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s">&times;</a>', wc_get_cart_remove_url($cart_item_key), esc_html__( 'Remove this item', 'wpgeneral' ) ), $cart_item_key ); ?>
						</div>
						
					</li>
					<?php 
					} else {
						?><li class="media last"><?php esc_html_e('view more on cart page', 'wpgeneral');?></li><?php 
						break;
					}
	...
	<?php else : ?>
		...
		<li class="empty"><?php esc_html_e( 'No products in the cart.', 'wpgeneral' ); ?></li>
...
<?php if ( sizeof( WC()->cart->get_cart() ) > 0 ) : ?>
	...
	<p class="total"><strong><?php esc_html_e( 'Subtotal', 'wpgeneral' ); ?>:</strong> <?php echo WC()->cart->get_cart_subtotal(); ?></p>

	<?php do_action( 'woocommerce_widget_shopping_cart_before_buttons' ); ?>

	<p class="buttons">
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button"><?php esc_html_e( 'View Cart', 'wpgeneral' ); ?></a>
		<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="button checkout"><?php esc_html_e( 'Checkout', 'wpgeneral' ); ?></a>
	</p>
