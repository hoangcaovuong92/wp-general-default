				...
				<div class ="breadcrumb_category" >
					...
					<?php
						if( isset($tvlgiao_wpdance_wd_data['wd_prod_cat_custom_content']) && strlen($tvlgiao_wpdance_wd_data['wd_prod_cat_custom_content']) > 0 ){
							...
							echo do_shortcode (stripslashes(htmlspecialchars_decode(base64_decode($tvlgiao_wpdance_wd_data['wd_prod_cat_custom_content']))) );
							echo "</div>";
						}
					?>
					...
					<?php 
						...
						if( absint($tvlgiao_wpdance_wd_data['wd_prod_cat_column']) > 0 ){
							$woocommerce_loop['columns'] = absint($tvlgiao_wpdance_wd_data['wd_prod_cat_column']);
						}
					...
					<div class="product-subcategories-slider wd-loading">
						<ul class="archive-product-subcategories <?php echo absint($tvlgiao_wpdance_wd_data['wd_prod_sub_cats_style'])? 'icon_style': '';?>">	
							<?php echo wp_kses_post($product_subcategories_html);?>
					...
					//<![CDATA[
						jQuery(document).ready(function() {
							"use strict";
							var temp_visible = <?php echo absint($tvlgiao_wpdance_wd_data['wd_prod_sub_cat_num']);?>;
