...
function wd_remove_color_theme(){
	...
	if($file !== $color_theme_saved ) {
		...
		if (file_exists($xml_file) && file_exists($img_file)) {
			...
			$res['status'] = 'error';
			$res['msg'] = esc_html__('Error: Color theme files are not exist.', 'wpgeneral');
		}
		
	} else {
		$res['status'] = 'error';
		$res['msg'] = esc_html__('Error: Color theme are using, you can\'t remove it.', 'wpgeneral');
	}
				...
				if(isset($header_datas['Slug']) && $header_datas['Slug'] !== '') {
					if(!file_exists(TVLGiao_Wpdance_XML_PATH . $header_datas['Slug'].'.xml')) {
						if(copy($tmp_file_png[0],TVLGiao_Wpdance_ADMIN_PATH .'assets/images/'.$header_datas['Slug'].'.png') 
							...
							$resturn['msg'] = esc_html__('Import success!', 'wpgeneral');
						} else {
							$resturn['status'] = 'error';
							$resturn['msg'] = esc_html__('Error: Import color theme not success.', 'wpgeneral');
						}
					} else {
						$resturn['status'] = 'error';
						$resturn['msg'] = esc_html__('Error: Color theme slug exist.', 'wpgeneral');
					}
				} else {
					$resturn['status'] = 'error';
					$resturn['msg'] = esc_html__('Error: XML syntax error.', 'wpgeneral');
				}
					
			} else {
				$resturn['status'] = 'error';
				$resturn['msg'] = esc_html__('Error: Extracting file is error.', 'wpgeneral');
			}
		} else {
			$resturn['status'] = 'error';
			$resturn['msg'] = esc_html__('Error: Upload file is error', 'wpgeneral');
		}
	}
