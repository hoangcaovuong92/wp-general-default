...
get_header(); 
 ...
 $yith_woocompare = TvlgiaoWpdanceClassNameVar::TVLGiao_wpdance_GetVar("yith_woocompare"); $fontend = $yith_woocompare->obj;
				...
				<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
					...
					<?php if ( count( $products ) ==0 ) : ?>
						...
						<tr class="no-products">
							<td><?php esc_html_e( 'No products added in the compare table.', 'wpgeneral' ) ?></td>
						</tr>
						...
						<tr class="remove">
							...
							<?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id();?>
								<td class="<?php echo esc_attr($product_class); ?>">
									<a href="<?php echo esc_url( add_query_arg( 'redirect', 'view', $fontend->remove_product_url( $product->get_id() ) ) );?>" class="remove" data-product_id="<?php echo esc_attr($product->get_id()); ?>">&times;</a>
								</td>
							<?php endforeach ?>
						</tr>
								...
								<td class="<?php echo esc_attr($product_class); ?>"><?php
									switch( $field ) {
										...
										case 'image':
											...
											break;
										case 'title':
											?><h3><a href="<?php echo esc_url(get_permalink($product->get_id()));?>" title="<?php echo esc_attr($product->fields[$field]);?>"><?php 
