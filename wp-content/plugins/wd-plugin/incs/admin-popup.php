...
if( !class_exists( 'WD_Admin_Popup' ) ) {
	class WD_Admin_Popup {
		...
		public function wd_popup_init(){
			$this->meta_post_pp_slug = TVLGiao_Wpdance_THEME_SLUG.'popup_configuration';
		...
		public function wd_popup_manage_enab_action(){
			...
			$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($id,$this->meta_post_pp_slug,true));
			$tvlgiao_wpdance_wd_data['popup_enable'] = $ena_val;
			
			$ret_str = serialize($tvlgiao_wpdance_wd_data);			
		...
		public function wd_popup_remove_action_row( $actions, $post ){
			if( $post->post_type === $this->post_type ) {
				unset( $actions['view'] );
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post->ID,$this->meta_post_pp_slug,true));
				if(isset($tvlgiao_wpdance_wd_data['popup_enable'])){
					$txt = absint($tvlgiao_wpdance_wd_data['popup_enable']) == 1? __('Disable', 'wpdance'): __('Enable', 'wpdance');
					$val = absint($tvlgiao_wpdance_wd_data['popup_enable']) == 0? 1: 0;
		...
		public function wd_popup_table_content( $column_name, $post_id) {
			if ($column_name == 'wd_status') {
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post_id,$this->meta_post_pp_slug,true));
				if( isset($tvlgiao_wpdance_wd_data['popup_enable']) && absint($tvlgiao_wpdance_wd_data['popup_enable']) == 1) {
				...
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post_id,$this->meta_post_pp_slug,true));
				if( isset($tvlgiao_wpdance_wd_data['popup_location'])) {
					switch( $tvlgiao_wpdance_wd_data['popup_location'] ) {
						case 'home': 
				...
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post_id,$this->meta_post_pp_slug,true));
				if( isset($tvlgiao_wpdance_wd_data['popup_cookie_delay']) && absint($tvlgiao_wpdance_wd_data['popup_cookie_delay']) > 0 ) {
					if( absint($tvlgiao_wpdance_wd_data['popup_cookie_delay']) <= 30 ) {
						echo "<label class=\"wd_label noti\">".absint($tvlgiao_wpdance_wd_data['popup_cookie_delay'])."m</label>";
					} else if( absint($tvlgiao_wpdance_wd_data['popup_cookie_delay']) <= 720 ) {
						echo "<label class=\"wd_label noti\">". (absint($tvlgiao_wpdance_wd_data['popup_cookie_delay'])/60) ."h</label>";
					} else {
				...
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post_id,$this->meta_post_pp_slug,true));
				$bg_color = '#ffffff';
				if(isset($tvlgiao_wpdance_wd_data['popup_thumb_color'])) {
					$bg_color = esc_attr($tvlgiao_wpdance_wd_data['popup_thumb_color']);
				}
				...
				$tvlgiao_wpdance_wd_data = unserialize(get_post_meta($post_id,$this->meta_post_pp_slug,true));
				if( isset($tvlgiao_wpdance_wd_data['popup_timeout_to_close']) && isset($tvlgiao_wpdance_wd_data['popup_timeout_delay']) && absint($tvlgiao_wpdance_wd_data['popup_timeout_to_close']) == 1 && absint($tvlgiao_wpdance_wd_data['popup_timeout_delay']) > 0 ) {
					if ($tvlgiao_wpdance_wd_data['popup_timeout_delay'] < 60) {
						echo "<label class=\"wd_label noti\">".absint($tvlgiao_wpdance_wd_data['popup_timeout_delay'])."s</label>";
					} else {
						$m = absint( absint($tvlgiao_wpdance_wd_data['popup_timeout_delay'])/60 );
						$s = absint($tvlgiao_wpdance_wd_data['popup_timeout_delay'])%60;
						$s = ( $s > 0 )? $s."s": "";
		...
		public function init_wp_script(){
			wp_enqueue_style( 'tvlgiao-wpdance-color-picker' );
			
			wp_enqueue_script('jquery');
			
			wp_register_style( 'tvlgiao-wpdance-wd_pp_style', WD_URL_ASSESTS . 'css/admin-pp.style.css', false, false );
			wp_enqueue_style( 'tvlgiao-wpdance-wd_pp_style' );
			
			wp_register_script('tvlgiao-wpdance-wd_popup_admin', WD_URL_ASSESTS . 'js/wd-admin-popup.js', array( 'tvlgiao-wpdance-color-picker' ), false, true);
			wp_enqueue_script('tvlgiao-wpdance-wd_popup_admin');
