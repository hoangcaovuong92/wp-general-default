					...
					<div class="item <?php echo $span_class ?><?php if( $i == 0 || $i % $columns == 0 ) echo ' first';?><?php if( $i == $num_count-1 || $i % $columns == $columns-1 ) echo ' last';?>">
						...
						<div class="item-content">
							<div class="post-info-thumbnail display-flex <?php if(!$thumbnail) echo "hidden-element"?>">
								...
								<a class="thumbnail effect_color" href="<?php the_permalink(); ?>">
									<?php 
										...
										the_post_thumbnail( array(350,220),array('class' => 'thumbnail-effect-1') );
									?>
							...
							if(absint($comments_count->approved) == 0) echo $comments_count->approved . ' ' . __('Comment', 'wpdance');
								...
								<p class="excerpt <?php if(!$excerpt) echo 'hidden-element'; ?>"><?php tvl_wpdance_the_excerpt_max_words($excerpt_words); ?></p>
	...
	function recent_portfolios_function($atts,$content = false){
		...
		if( have_posts() ) :
			...
			while(have_posts()) {
				...
				$url_video = get_post_meta($post->ID,TVLGiao_Wpdance_THEME_SLUG.'video_portfolio',true);
				...
				$portfolio_slider = get_post_meta($post->ID,TVLGiao_Wpdance_THEME_SLUG.'_portfolio_slider',true);
				...
				<li class="item <?php echo $span_class ?><?php if( $i == 0 || $i % $columns == 0 ) echo ' first';?><?php if( $i == $num_count-1 || $i % $columns == $columns-1 ) echo ' last';?>">
					<div>
						<div class="image <?php if(!$thumbnail) echo "hidden-element"?>">
							...
							<?php }else{ ?>
								<?php 
									the_post_thumbnail('tvlgiao_wpdance_blog_shortcode',array('class' => 'thumbnail-effect-1'));
								?>
							<?php }?>
						</div>
