			...
			<div class="product-bigger">
					...
					<div class="widget_product_slider wd-loading" id="<?php echo esc_attr($_random_id);?>">
						...
						<?php while ($best_selling->have_posts()) : $best_selling->the_post();?>
							...
							?>
							<div class="thumbnail"><a class="<?php echo (!wp_is_mobile())? "wd_brand_product_slide_func1": ''; ?>" title="<?php echo esc_attr($product->get_title());?>" href="<?php echo esc_url($prouct_link);?>" data-prod_id="<?php echo esc_attr($product->get_id());?>"><?php echo $product->get_image(array( 150, 150 )); ?></a></div>
						<?php endwhile;?>
						</div><!--.products-->
					</div>	
				</div>						
