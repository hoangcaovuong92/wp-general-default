		...
		if ( $id_sale_product ) {
			...
			for ( $i = 0; $i < count( $id ); $i ++ ) {
				...
				if ( trim( $product->get_type() ) != 'variable' ) {
					...
					$_turn_off_countdown = get_post_meta( $product->get_id(), "_turn_off_countdown", true );
					...
					$shortcode .= '<li class="product product-' . $product->get_id() . '">';
					if ( trim( $ob_product_show_linkable ) == 'yes' ) {
						$shortcode .= '<a href="' . get_permalink( $product->get_id() ) . '">';
					}
					...
					/*Get time schedule*/
					$discount      = get_post_meta( $product->get_id(), "_quantity_discount", true );
					$sale          = get_post_meta( $product->get_id(), "_quantity_sale", true );
						...
						if ( $time_end && $time_from ) {
							...
							switch ( $ob_time_show ) {
								case 1:
									...
									$shortcode .= '<div class="shortcode_product_' . $product->get_id() . ' shortcode_product"></div>
                                </div>
                                ';
									break;
								case 2:
								...
								<span>' . $sale . '/' . $discount . __( ' sold', 'woosalescountdown' ) . '</span>';
									...
									$shortcode .= '<div class="shortcode_product_' . $product->get_id() . ' shortcode_product"></div></div>';
							}
							...
							if ( ! empty( $time_end ) && ! empty( $time_from ) ) {
								...
								<script type="text/javascript">
									jQuery(function () {
										jQuery(".shortcode_product_' . $product->get_id() . '").mbComingsoon({ expiryDate: new Date(' . date( "Y", $time ) . ', ' . ( date( "m", $time ) - 1 ) . ', ' . date( "d", $time ) . ', ' . date( "G", $time ) . ',' . date( "i", $time ) . ', ' . date( "s", $time ) . '),speed: 500, gmt:' . get_option( 'gmt_offset' ) . $date_text . $localization . ' });
									});
								</script>';
