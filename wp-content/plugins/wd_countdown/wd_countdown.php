	...
	if (!class_exists('WD_Countdown')) {
		...
		class WD_Countdown {
			...
			function setup_countdown() {			
				...
				add_action( 'woocommerce_single_product_summary', array( $this, 'countdown_set_default_view' ), 1 );
			}
			...
			function setup_scripts_styles() {
				wp_enqueue_style( 'tvlgiao-wpdance-jquery.countdown', plugins_url( '/assets/css/countdown.css', __FILE__ ) );
			}
			function setup_scripts_script() {
				wp_enqueue_script( 'tvlgiao-wpdance-plugin.countdown', plugins_url( '/assets/js/jquery.plugin.min.js', __FILE__ ), array( 'jquery' ) );
				wp_enqueue_script( 'tvlgiao-wpdance-jquery.countdown', plugins_url( '/assets/js/jquery.countdown.min.js', __FILE__ ), array( 'jquery' ) );
				wp_enqueue_script( 'tvlgiao-wpdance-script.countdown', plugins_url( '/assets/js/countdown.js', __FILE__ ), array( 'jquery' ) );
			}
		...
		$tvlgiao_wpdance_WD_Countdown = new WD_Countdown();
	}
}
